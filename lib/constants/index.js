...
export function generateUniqueID() {
  const chars = "abcdefghijklmnopqrstuvwxyz0123456789"; // Lowercase + Numbers
  const fixedPart = "43d"; // First 3 characters remain constant

  // Generate a changing fourth character based on the day
  const date = new Date();
  const dayOfYear = Math.floor(
    (date - new Date(date.getFullYear(), 0, 0)) / 86400000
  ); // Day of the year (1-365)
  const changingChar = chars[dayOfYear % chars.length]; // Pick a character based on the day
  ...
  return fixedPart + changingChar + randomStr; // Combine parts
}
